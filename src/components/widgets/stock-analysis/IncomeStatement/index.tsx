...
import {
  ...
  transformApiIncomeStatementResponse,
  type IncomeRow
} from './api';
...
export function IncomeStatement({ onSettings, onRemove, onFullscreen, settings = {} }: IncomeStatementProps) {
  ...
  // Fetch income statement when symbol changes
  useEffect(() => {
    const loadIncomeStatement = async () => {
      const cacheKey = widgetDataCache.generateKey('income-statement', { symbol });
      const cachedData = widgetDataCache.get<{ data: IncomeRow[]; years: string[] }>(cacheKey);
      
      if (cachedData) {
        setIncomeData(cachedData.data);
        setYears(cachedData.years);
        setIsLoading(false);
        return;
      }

      console.log('🔄 [Income Statement Component] Starting to load income statement for:', symbol);
      setIsLoading(true);
      setError(null);
      
      try {
        const apiResponse = await fetchIncomeStatement(symbol);
        
        if (!apiResponse) {
          console.error('❌ [Income Statement Component] API response is null');
          setError('Failed to load income statement: No response');
          setIsLoading(false);
          return;
        }
        
        if (!apiResponse.success || !apiResponse.data) {
          console.error('❌ [Income Statement Component] API response not successful');
          setError('Failed to load income statement: API returned error');
          setIsLoading(false);
          return;
        }
        
        console.log('✅ [Income Statement Component] Transforming data');
        const transformed = transformApiIncomeStatementResponse(apiResponse.data);
        console.log('✅ [Income Statement Component] Transformed data:', transformed);
        
        setIncomeData(transformed.data);
        setYears(transformed.years);
        widgetDataCache.set(cacheKey, { data: transformed.data, years: transformed.years });
        setIsLoading(false);
      } catch (err) {
        console.error('❌ [Income Statement Component] Error loading income statement:', err);
  ...
  const formatValue = (value: number | null): string => {
    ...
    // For EPS values (small decimals)
    if (absValue < 100) {
      return value < 0 ? `(${absValue.toFixed(2)})` : absValue.toFixed(2);
    }
    
    // For large values, format as thousands with commas
  ...
  return (
    <div className="h-full bg-widget-body border border-border rounded-none flex flex-col">
      <WidgetHeader 
        ...
        helpContent="Annual income statement showing revenue, expenses, and net income. All values in millions USD except EPS."
      />
      
      <WidgetSettingsSlideIn
        isOpen={isSettingsPanelOpen}
        onClose={() => setIsSettingsPanelOpen(false)}
        widgetType="income-statement"
        widgetPosition=""
