...
import { fetchPositionBook, type PositionBookResponse } from './api';
...
export function PositionBookWidget({
  onRemove,
  onSettings,
  onFullscreen,
  settings
  ...
  const [data, setData] = useState<PositionBookResponse | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await fetchPositionBook(symbol, 'positionbook');
      setData(response);
    } catch (err) {
      console.error('Error fetching position book data:', err);
    ...
    if (rect.width === 0 || rect.height === 0) return;
      ...
      priceLevels.forEach((level, index) => {
        ...
        if (labelIndices.has(index) && Math.abs(level.price) > 0.0001) {
          ...
          ctx.textAlign = 'right';
          ctx.fillText(level.price.toFixed(4), padding.left + 10, y + 5);
        }
      });
  ...
  return (
    <div className="flex flex-col h-full bg-widget-body border border-border rounded-none overflow-hidden">
      <WidgetHeader
        ...
        helpContent="Displays real-time position book showing order flow and market depth with bid/ask levels. Red bars indicate bid volumes (left), teal bars indicate ask volumes (right)."
      />
      
      <div className="flex-1 overflow-hidden relative" ref={containerRef}>
        {isLoading ? (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-muted-foreground text-sm">Loading position book data...</div>
          </div>
        ) : error ? (
          <div className="absolute inset-0 flex items-center justify-center">
...
export default PositionBookWidget;
